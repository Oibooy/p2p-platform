...
const {
  exportUsers,
  exportDeals,
} = require("../../infrastructure/exportService");
...
router.get("/users/:id", verifyToken, checkRole("admin"), async (req, res) => {
  try {
    const user = await UserRepository.findById(req.params.id).select(
      "-password",
    );
    if (!user) {
    ...
    try {
      ...
      if (newRate < 0 || newRate > 1000) {
        return res
          ...
          .json({
            error:
              "Commission rate must be between 0 and 1000 basis points (0-10%)",
          });
      }
